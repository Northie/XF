...
class elFinder {
	...
	public function __construct($opts) {
		...
		if (isset($opts['roots']) && is_array($opts['roots'])) {
			...
			$class = '\libs\elfinder\elFinderVolume'.(isset($o['driver']) ? $o['driver'] : '');
	...
	public function exec($cmd, $args) {
		...
		$rp = $this->realpath($_GET['target']);
		
		$path = realpath(dirname($rp)."/".basename($rp));
		
		$path = str_replace("\\", "/", $path);
		
		$result['path'] = str_replace($_SERVER['DOCUMENT_ROOT'],"",$path);
